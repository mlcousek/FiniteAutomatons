...
// Arrange
 ...
 // Should NOT show Result badge yet (input not fully read)
    ...
    [Fact]
    public async Task ResultDisplay_AfterSteppingThroughEntireInput_Accepted_ShowsAccepted()
 {
        // Arrange
     var client = GetHttpClient();
        var model = new AutomatonViewModel
...
Input = "aa"
        ...
        // Act - Start and step through entire input
        var startResponse = await PostAutomatonAsync(client, "/Automaton/Start", model);
        var result1 = await DeserializeResponseAsync(startResponse);
        
        var step1Response = await PostAutomatonAsync(client, "/Automaton/StepForward", result1);
        var result2 = await DeserializeResponseAsync(step1Response);
        
  var step2Response = await PostAutomatonAsync(client, "/Automaton/StepForward", result2);
        var html = await step2Response.Content.ReadAsStringAsync();

        // Assert
        step2Response.StatusCode.ShouldBe(HttpStatusCode.OK);
      
        var finalResult = await DeserializeResponseAsync(step2Response);
        finalResult.Position.ShouldBe(finalResult.Input!.Length);
        finalResult.IsAccepted.ShouldBe(true);
    ...
    [Fact]
    public async Task ResultDisplay_AfterSteppingThroughEntireInput_Rejected_ShowsRejected()
    {
        // Arrange
        var client = GetHttpClient();
      var model = new AutomatonViewModel
  ...
  [
    ...
    new() { Id = 2, IsStart = false, IsAccepting = true }
     ...
     new() { FromStateId = 1, ToStateId = 2, Symbol = 'a' }
      ...
      Input = "ab" // b has no transition, will reject
     };

   // Act - Start and step through until rejection
        var startResponse = await PostAutomatonAsync(client, "/Automaton/Start", model);
      var result1 = await DeserializeResponseAsync(startResponse);
 ...
 step2Response.StatusCode.ShouldBe(HttpStatusCode.OK);
        ...
        var finalResult = await DeserializeResponseAsync(step2Response);
 finalResult.IsAccepted.ShouldBe(false);
